...
NetSpeedExtension.prototype = {
  _createLabelStyle: function(labelName) {
    ...
    if (!!labelName) {
      ...
      else if (this._useCustomFontColours)
  ...
  _loadSettings: function() {
    ...
    this._customSpeedDownColour = this._settings.get_string(
      "custom-speed-down-colour"
    );
    this._customSpeedDownDecoration = this._settings.get_string(
      "custom-speed-down-decoration"
    );
    this._customSpeedTotalColour = this._settings.get_string(
      "custom-speed-total-colour"
    );
    this._customSpeedTotalDecoration = this._settings.get_string(
      "custom-speed-total-decoration"
    );
    this._customSpeedUpColour = this._settings.get_string(
      "custom-speed-up-colour"
    );
    this._customSpeedUpDecoration = this._settings.get_string(
      "custom-speed-up-decoration"
    );
    this._customUsageTotalColour = this._settings.get_string(
      "custom-usage-total-colour"
    );
    this._customUsageTotalDecoration = this._settings.get_string(
    ...
    this._useCustomDecorations = this._settings.get_boolean(
      "use-custom-decorations"
    );
    this._useCustomFontColours = this._settings.get_boolean(
      "use-custom-font-colours"
    );
    this._useCustomFontFamily = this._settings.get_boolean(
      "use-custom-font-family"
    );
    this._useCustomFontSize = this._settings.get_boolean(
  ...
  _update: function() {
    ...
    if (this._runNum > this._currentRunNum) {
      ...
      Main.Mainloop.timeout_add_seconds(
  ...
  _updateSpeeds: function() {
    ...
    for (let i = 0; i < this._trackedInterfaces.length; i++) {
      ...
      let totalUp =
        interfaceData.totalTransmitted - interfaceData.initialTransmitted;
  ...
  _onButtonClicked: function(button, event) {
    if (event.get_button() == 3) {
      // Clear counter on right click
      for (let i = 0; i < this._interfaceData.length; i++) {
        this._interfaceData[i].initialReceived = this._interfaceData[
          i
        ].totalReceived;
        this._interfaceData[i].initialTransmitted = this._interfaceData[
          i
        ].totalTransmitted;
      }
  ...
  _onSettingsChanged: function(settings, key) {
    switch (key) {
      ...
      case "custom-speed-down-colour":
        this._customSpeedDownColour = tsettings.get_string(
          "custom-speed-down-colour"
        );
        this._downLabel.set_style(this._createLabelStyle("down"));
        break;
      ...
      case "custom-speed-total-colour":
        this._customSpeedTotalColour = settings.get_string(
          "custom-speed-total-colour"
        );
        this._totalLabel.set_style(this._createLabelStyle("total"));
        break;
      ...
      case "custom-speed-up-colour":
        this._customSpeedUpColour = settings.get_string(
          "custom-speed-up-colour"
        );
        this._upLabel.set_style(this._createLabelStyle("up"));
        break;
      ...
      case "custom-usage-total-colour":
        this._customUsageTotalColour = settings.get_string(
          "custom-usage-total-colour"
        );
        this._usageLabel.set_style(this._createLabelStyle("usage"));
        break;
      ...
      case "use-custom-font-colours":
        this._useCustomFontColours = settings.get_boolean(
          "use-custom-font-colours"
        );
        this._setAllLabelStyles();
        break;
      case "use-custom-decorations":
        this._useCustomDecorations = settings.get_boolean(
          "use-custom-decorations"
        );
        if (this._useCustomDecorations) {
          this._speedDownDecoration = settings.get_string(
            "custom-speed-down-decoration"
          );
          this._speedUpDecoration = settings.get_string(
            "custom-speed-up-decoration"
          );
          this._speedTotalDecoration = settings.get_string(
            "custom-speed-total-decoration"
          );
          this._usageTotalDecoration = settings.get_string(
            "custom-usage-total-decoration"
          );
        } else {
      ...
      case "use-custom-font-family":
        this._useCustomFontFamily = settings.get_boolean(
          "use-custom-font-family"
        );
        this._setAllLabelStyles();
        break;
  ...
  enable: function() {
    ...
    this._buttonSignal = this._button.connect(
      "button-press-event",
    ...
    if (timeDiff > 1 || timeDiff < -1) {
      // timeDiff accurate to +- 1 second
      this._settings.set_int("last-boot-time", thisBootTime);
      this._settings.apply();
    } else {
      let initialReceivedValues = this._settings.get_strv(
        "initial-receive-counts"
      );
      let initialTransmittedValues = this._settings.get_strv(
        "initial-transmit-counts"
      );

      // This won't work with different lengths, so give up.
      if (
        (!initialReceivedValues.length == initialTransmittedValues.length) ==
      ...
      for (let i = 0; i < this._trackedInterfaces.length; i++) {
        ...
        if (
          initialReceived > interfaceData.initialReceived &&
